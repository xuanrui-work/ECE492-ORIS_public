...
class SimpleORIS:
    def __init__(self, resolution: Tuple[int], fps: int):
        ...
        self.camera_thread = camera_stream.CameraStreamThread(self.resolution, self.fps)
    ...
    def process_image(self):
        ...
        if self.run_inference:
            ...
            frame[y:y+h, x:x+w] = self.make_inference(frame[y:y+h, x:x+w])
            frame[y:y+h, x:x+w] = self.draw_inference_box(frame[y:y+h, x:x+w])
        
        self.fps_counter.update()
        self.fps_counter.draw_on_img(frame)
